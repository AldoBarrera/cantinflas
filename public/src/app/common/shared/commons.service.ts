...
export class CommonsService {
  ...
  protected urlModule = '/api/v1/ipmonitor';
  protected urlRelative = '/commons';
  ...
  createRemoteEvents() {
    ...
    CommonsUtil.getRemoteEvents(this, this.name, this.remoteMethods, this);  
  ...
  remoteMethodDelete(data:any, self) {
    self.data.removeElementById(data);
  }
  ...
  getAllCustomDataAsObserver(urlRealtive, fields, populated:Boolean = null): Observable<CommonsModel[]> {
    let params = this.buildParams(fields, populated);
    return this.http.get<CommonsModel[]>(environment.appConfig.url + this.urlModule + urlRealtive, {params})
    .pipe(
      tap(CommonsModel => this.log('fetched Data')),
      catchError(this.handleError('getData', []))
    ); 
  ...
  getIdCustomDataAsObserver(urlRealtive, fields, id, populated:Boolean = null): Observable<CommonsModel[]> {
    let params = this.buildParams(fields, populated);
    urlRealtive = `${urlRealtive}/${id}`;
    return this.http.get<CommonsModel[]>(environment.appConfig.url + this.urlModule + urlRealtive, {params})
    .pipe(
      tap(CommonsModel => this.log('fetched Data')),
      catchError(this.handleError('getData', []))
